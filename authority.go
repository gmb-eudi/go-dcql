...
// up front — unknown type = query invalid).
func authorityMatches(tas []TrustedAuthority, ref AuthorityRef) bool {
	...
	for i := range tas {
		...
		case AuthorityTypeAKI:
			have = ref.AKIs // [OID4VP §6.1.1]: base64url KeyIdentifier of an AKI in the chain
		case AuthorityTypeETSITL:
			have = ref.TrustedListURIs // [OID4VP §6.1.1]: ETSI TS 119 612 trusted list identifier
		case AuthorityTypeOpenIDFederation:
			have = ref.FederationIDs // [OID4VP §6.1.1]: OpenID Federation Entity Identifier
		default:
			continue
		}
